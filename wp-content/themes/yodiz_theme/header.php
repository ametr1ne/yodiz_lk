...
if ( $post->post_name === 'user_account' ) {
	if ( isset( $_SESSION['parentID'] ) ) {
		...
		$courseID = key( $userMeta );
	}
} else {
	$courseID = get_post_ancestors( $post->ID )[1]; // ID корневой страницы для курсов
	$_SESSION['parentID'] = $courseID;
}
foreach ( json_decode( get_user_meta( get_current_user_id(), 'lessonsProgress' )[0] )->$courseID->list as $course ) {
	foreach ( $course->lessons as $lesson ) {
		if ( $post->ID === $lesson->id && ! $lesson->isCompleted ) {
			header( home_url() );
		}
	}
}
...
<head>
    ...
    <link rel="stylesheet" href="<?= get_template_directory_uri() ?>/assets/scss/lessonPage.css"/>
    <link rel="stylesheet" href="<?= get_template_directory_uri() ?>/assets/css/simplebar.min.css"/>
    <link rel="stylesheet" href="<?= get_template_directory_uri() ?>/assets/css/bonus_banner.css"/>
...
<header class="header">
    ...
    <div class="header__wrapper">
        <div class="header__left">
            <div class="header__logo">
                <svg width="66" height="50" viewBox="0 0 66 50" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M48.6918 0H16.7627L0 16.9355L32.7273 50L65.4545 16.9355L48.6918 0ZM37.4687 24.2097V36.2903H27.9858V24.2097L16.7627 12.8548L22.3024 7.25806L32.7273 17.7903L43.1521 7.25806L48.6918 12.8548L37.4687 24.2097Z" fill="white"/>
                </svg>
            </div>
            <a class="header__link" href="<?= home_url() ?>/courses">Мои курсы</a>
            <a class="header__link" href="<?= home_url() ?>/courses/designschedule/">Расписание занятий</a>
        </div>
        <div class="header__btns">
            ...
            <a href="<?php echo wp_logout_url( home_url() ); ?>" class="btn header-btn d_greyborder-btn logout-btn">Выйти</a>
        </div>
    </div>
</header>
