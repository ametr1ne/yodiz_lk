<?php
/**
 * Template Name: Страница всех доступных курсов *
 */
...
<head>
    ...
    <link rel="stylesheet" href="<?= get_template_directory_uri() ?>/assets/scss/coursesPage.css"/>
    <link rel="icon" type="image/x-icon" href="<?= get_template_directory_uri() ?>/assets/img/favicon.svg">
    <title>Курсы от Yodiz School</title>
	<?php wp_head(); ?>
</head>
<body>
<div class="dark"></div>
...
<header class="header">
    ...
    <div class="header__wrapper">
        ...
        <div class="header__btns">
            <a href="<?= home_url() ?>/user_account/" class="btn header-btn user-btn white-btn">
        ...
        <div class="content__wrapper">
			...
			if ( isset( $allCourses ) ) {
				foreach ( $allCourses as $course ) {
					...
					$allSubCourses = get_field( 'subcourses', $course->ID ); ?>

                    <div class="courses-item content-item">
                        <div class="item__header">

                            <!--						--><?php //include 'componets/progressBlock.php' ?>

                        </div>
                        <hr/>
                        <div class="courses-box">
							<?php
							if ( isset( $allSubCourses ) ) {
								foreach ( $allSubCourses as $subCourse ) {

									$deal = new BitrixBackend_Deal();

									$courseDeal = $deal->getDeal( $userID, $subCourse );
                                    ...
                                    <div class="courses-box__item <?php if ( ! get_field( 'course_paid', $subCourse ) ) {
										echo 'free-course';
									} else echo 'paid-course' ?>">
                                        <div class="course-category">
                                            <p><?php if ( get_field( 'course_paid', $subCourse ) ) {
													echo 'Основной курс';
												} else echo 'Подготовительный курс' ?></p>
                                            <p><?= get_field( 'course_duration', $subCourse ) ?></p>
                                        </div>
                                        <h2 class="subcourse-name"><?= get_field( 'course_name', $subCourse ) ?></h2>
                                        <div class="course-info">
                                            <p class="access">
												<?php if ( ! get_field( 'course_paid', $subCourse ) ) {
													?>
													...
													if ( empty( $courseDeal ) ) { ?>
                                                        <svg class="access__paid" width="13" height="16"
                                                             viewBox="0 0 13 16"
                                                             fill="none"
                                                             xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M7.00635 9.96875H5.55615V12.9688H7.00635V9.96875Z"
                                                                  fill="#696978"/>
                                                            <path fill-rule="evenodd" clip-rule="evenodd"
													...
													<?php } else {
														if ( $courseAccess === '1' ) { ?>
                                                            <svg class="access__free" width="9" height="9"
                                                                 viewBox="0 0 9 9"
                                                                 fill="none"
                                                                 xmlns="http://www.w3.org/2000/svg">
                                                                <circle cx="4.28125" cy="4.99463" r="4" fill="#8DE300"/>
										...
										<?php if ( ! get_field( 'course_paid', $subCourse ) ) { ?>
                                            <a href="<?= home_url() ?>/schedule?course=<?php echo $course->ID ?>" class="btn greyborder-btn">
											...
											if ( empty( $courseDeal ) ) { ?>
                                                <a class="btn yellow-btn">
                                                    <svg width="14" height="17" viewBox="0 0 14 17" fill="none"
                                                         xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M7.93646 10.2493H6.38959V13.4493H7.93646V10.2493Z"
                                                              fill="black"/>
                                                        <path fill-rule="evenodd" clip-rule="evenodd"
                                                              d="M13.3448 8.64929C13.3448 8.62559 13.3448 8.60188 13.3448 8.57818V7.04929H11.798H11.0245V4.65522C11.0245 2.43892 9.29566 0.649292 7.15733 0.649292C5.019 0.649292 3.29014 2.43892 3.29014 4.65522V5.46114H4.84839V4.65522C4.84839 3.32781 5.88344 2.24929 7.16871 2.24929C8.45398 2.24929 9.48902 3.32781 9.48902 4.65522V7.04929H8.4881H6.15641H5.82656H4.84839H3.30152H2.84655H2.52808H0.981201V8.57818C0.981201 8.60188 0.981201 8.62559 0.981201 8.64929V15.0493C0.981201 15.073 0.981201 15.0967 0.981201 15.1204V16.6374H2.34609C2.40296 16.6493 2.45983 16.6493 2.52808 16.6493H11.798C11.8548 16.6493 11.9231 16.6493 11.9799 16.6374H13.3448V15.1204C13.3448 15.0967 13.3448 15.073 13.3448 15.0493V8.64929ZM2.52808 15.0493V8.64929H11.798V15.0493H8.4881H5.83794H2.52808Z"
                                                              fill="black"/>
                                                    </svg>
                                                    Оплатить участие</a>
                                                <a href="" class="btn d_greyborder-btn details-btn">
                                                    Подробная информация о курсе
                                                </a>
											<?php } else { if ( $courseAccess === '1' ) { ?>
                                                <a href="<?= home_url() ?>/schedule?course=<?php echo $course->ID ?>" class="btn white-btn">Открыть расписание</a>
                                                <a class="btn d_grey-btn">Перейти на тариф с обратной связью
                                                    <span class="teachers">
                                                        <img class="dima" src="<?= get_template_directory_uri() ?>/assets/img/1.png" alt="1">
                                                        <img class="lika" src="<?= get_template_directory_uri() ?>/assets/img/2.png" alt="2">
                                                        <img class="lesha" src="<?= get_template_directory_uri() ?>/assets/img/3.png" alt="3">
                                                    </span>
                                                </a>
												<?php }
                                                if ($courseAccess === '2') { ?>
...
<script src="<?= get_template_directory_uri() ?>/assets/js/coursesPage.js"></script>
